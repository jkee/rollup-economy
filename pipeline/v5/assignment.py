#!/usr/bin/env python3
"""Render isolated v5 researcher and validator assignments.
...
def research_prompt(
    ...
    return f"""You are the sole researcher for one v5 industry record.
...
- pipeline/v5/research_brief.md
- pipeline/v5/research_packet.schema.json
...
above exactly. After writing it, self-check only with:
  python3 pipeline/v5/build.py validate {run_dir.relative_to(ROOT)}/packet.json
...
def review_prompt(run_dir: Path, review_date: str, model_id: str) -> str:
    ...
    return f"""You are the fresh, isolated validator for one finalized v5 record.
...
- {relative}/score.json
- {relative}/memo.md
- pipeline/v5/methodology.md
- pipeline/v5/validator_brief.md
- pipeline/v5/review.schema.json
...
The orchestrator ran `python3 pipeline/v5/build.py check {relative}`. Copy this
