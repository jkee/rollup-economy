#!/usr/bin/env python3
"""v5 sentinels: scoring boundaries, evidence handling, packet/review contracts,
and the deterministic publication gate."""
...
from build import (
    build_site, check, finalize, finalize_path, render_memo, validate_packet,
    validate_review,
)
...
def packet(**overrides):
    base = {
        "identity": {"naics": "541512", "title": "Synthetic", "run_id": "t1",
                     ...
                     "methodology_version": "5.0", "attempt": 1},
...
def review_for(p, mechanics, **overrides):
    r = {"run_id": p["identity"]["run_id"], "naics": p["identity"]["naics"],
         ...
         "methodology_version": "5.0", "outcome": "publishable_with_caveats",
...
class PublicationGateSentinels(unittest.TestCase):
    """End-to-end: nothing reaches the site without integrity + a valid review."""

    def _setup(self, tmp: Path):
        run_dir = tmp / "runs" / "541512" / "t1"
        run_dir.mkdir(parents=True)
        (run_dir / "packet.json").write_text(json.dumps(packet(), indent=1), encoding="utf-8")
        finalize_path(run_dir / "packet.json")
        mechanics = check(run_dir)
        self.assertEqual(mechanics["errors"], [], mechanics)
        return run_dir, mechanics

    def test_unreviewed_is_excluded_and_valid_review_publishes(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            run_dir, mechanics = self._setup(tmp)
            site = build_site(False, runs=tmp / "runs", site_out=tmp / "six.json")
            self.assertEqual((site["summary"]["published"], site["summary"]["excluded"]), (0, 1))
            self.assertEqual(site["exclusions"][0]["outcome"], "unreviewed")
            (run_dir / "review.json").write_text(
                json.dumps(review_for(packet(), mechanics)), encoding="utf-8")
            site = build_site(False, runs=tmp / "runs", site_out=tmp / "six.json")
    ...
    def test_review_of_stale_artifacts_fails_closed(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            run_dir, mechanics = self._setup(tmp)
            stale = dict(mechanics, artifacts_sha256="c" * 64)
            (run_dir / "review.json").write_text(
                json.dumps(review_for(packet(), stale)), encoding="utf-8")
            with self.assertRaises(SystemExit):
                build_site(False, runs=tmp / "runs", site_out=tmp / "six.json")

    def test_malformed_accepted_review_fails_closed(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            run_dir, mechanics = self._setup(tmp)
            (run_dir / "review.json").write_text(json.dumps(
                {"outcome": "publishable"}), encoding="utf-8")
            with self.assertRaises(SystemExit):
                build_site(False, runs=tmp / "runs", site_out=tmp / "six.json")

    def _add_attempt(self, tmp: Path, run_id: str, attempt: int):
        p = packet(**{"identity.run_id": run_id, "identity.attempt": attempt})
        run_dir = tmp / "runs" / "541512" / run_id
        run_dir.mkdir(parents=True)
        (run_dir / "packet.json").write_text(json.dumps(p, indent=1), encoding="utf-8")
        finalize_path(run_dir / "packet.json")
        return p, run_dir, check(run_dir)

    def test_remediation_flow_and_latest_attempt_selection(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            p1, dir1, mech1 = self._add_attempt(tmp, "t1", 1)
            (dir1 / "review.json").write_text(json.dumps(review_for(
                p1, mech1, outcome="reject", material_findings=[
                    {"category": "unsupported_claim", "finding": "x",
                     "materiality": "correcting q would change the base tier"}])),
                encoding="utf-8")
            # Rejected only -> excluded, with the findings surfaced.
            site = build_site(False, runs=tmp / "runs", site_out=tmp / "six.json")
            self.assertEqual(site["summary"]["published"], 0)
            self.assertEqual(site["exclusions"][0]["outcome"], "reject")
            self.assertTrue(site["exclusions"][0]["material_findings"])
            # Accepted attempt 2 -> published, exclusions empty.
            p2, dir2, mech2 = self._add_attempt(tmp, "t2", 2)
            (dir2 / "review.json").write_text(json.dumps(review_for(p2, mech2)), encoding="utf-8")
            site = build_site(False, runs=tmp / "runs", site_out=tmp / "six.json")
            self.assertEqual(site["summary"]["published"], 1)
            self.assertEqual(site["records"][0]["run_id"], "t2")
            self.assertEqual(site["exclusions"], [])
            score = json.loads((dir2 / "score.json").read_text())
            self.assertEqual(score["envelope"]["interval_scope"],
                             "research-input sensitivity with dataset anchors (l, n) held fixed")

    def test_duplicate_and_excess_attempts_fail_closed(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            p1, dir1, mech1 = self._add_attempt(tmp, "t1", 1)
