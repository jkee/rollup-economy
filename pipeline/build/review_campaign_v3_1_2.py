...
def phase4_status(finalize=False):
    if finalize:
        return True, []
    return False, ["update final documentation and push validated checkpoint commits to origin"]


def finalization_state_errors(status_output, head, origin_head):
    errors = []
    if status_output.strip():
        errors.append("working tree must be clean before --finalize")
    if not head or not origin_head:
        errors.append("HEAD and origin/main must both resolve before --finalize")
    elif head != origin_head:
        errors.append("HEAD must already equal origin/main before --finalize")
    return errors


def repository_finalization_errors(repo_root):
    def git(*args):
    ...
    status_rc, status = git("status", "--porcelain")
    head_rc, head = git("rev-parse", "HEAD")
    origin_rc, origin_head = git("rev-parse", "origin/main")
    errors = []
    if status_rc or head_rc or origin_rc:
        errors.append("git status/HEAD/origin/main checks must succeed before --finalize")
    errors.extend(finalization_state_errors(status, head, origin_head))
    return errors


def build_campaign_consistency_errors(report, build_result):
    expected = {row["naics"]: row for row in report["records"] if row["kind"] == "fleet"}
    flags = {row["naics"]: row for row in build_result["flags"]["records"]}
    site = {row["naics"]: row for row in build_result["site"]["records"]}
    errors = []
    if set(flags) != set(expected):
        errors.append(
            "build fleet codes differ from campaign manifest: expected=%s actual=%s"
            % (sorted(expected), sorted(flags))
        )
    for naics in sorted(set(expected) & set(flags)):
        if flags[naics].get("run_id") != expected[naics]["run_id"]:
            errors.append(
                "%s: build selected run_id %r but campaign selected %r"
                % (naics, flags[naics].get("run_id"), expected[naics]["run_id"])
            )
    expected_published = {
        naics for naics, row in expected.items()
        if row["outcome"] in ("publishable", "publishable_with_caveats")
    }
    expected_rejected = {
        naics for naics, row in expected.items() if row["outcome"] in ("reject", "invalid")
    }
    if set(site) != expected_published:
        errors.append(
            "build publication set differs from campaign: expected=%s actual=%s"
            % (sorted(expected_published), sorted(site))
        )
    if set(build_result["stats"].get("rejected", [])) != expected_rejected:
        errors.append(
            "build rejection set differs from campaign: expected=%s actual=%s"
            % (sorted(expected_rejected), sorted(build_result["stats"].get("rejected", [])))
        )
    if build_result["stats"].get("pending_review"):
        errors.append("final build contains pending reviews")
    return errors
...
def render_report(report):
    lines = [
        ...
        "## Phase status",
        "",
        "- Campaign closed: %s" % ("yes" if report["campaign_closed"] else "no"),
        "- Phase 4 complete: %s" % ("yes" if report["phase4_complete"] else "no"),
    ]
    if report["phase4_remaining_actions"]:
        lines.append("- Remaining actions: %s" % "; ".join(report["phase4_remaining_actions"]))
    else:
        lines.append("- Remaining actions: none")
    lines.extend(["", "## Exact counts", "", "- Planned: %d fleet + %d golden = %d" % (report["planned"]["fleet"], report["planned"]["golden"], report["planned"]["total"]), "- Attempts: %d initial + %d remediation = %d" % (report["attempt_counts"]["initial"], report["attempt_counts"]["remediation"], report["attempt_counts"]["total"]), "- Published: %d" % report["published"], "- Excluded: %d" % report["excluded"]])
...
def deterministic_verification(repo_root):
    ...
    for command in commands:
        ...
        results.append(verification_result(command, completed.returncode, completed.stdout, completed.stderr))
    return results


def verification_result(command, returncode, stdout, stderr):
    # Successful test output commonly includes wall-clock timings. Exclude that
    # non-semantic noise so an identical close rerenders byte-for-byte.
    return {
        "command": " ".join(command),
        "returncode": returncode,
        "stdout_tail": "" if returncode == 0 else stdout[-2000:],
        "stderr_tail": "" if returncode == 0 else stderr[-2000:],
    }
...
def main(argv=None):
    ...
    close.add_argument(
        "--finalize",
        action="store_true",
        help="mark Phase 4 complete after final documentation is current",
    )
    args = parser.parse_args(argv)
    if args.command == "close" and args.finalize:
        finalize_errors = repository_finalization_errors(args.repo_root)
        if finalize_errors:
            for error in finalize_errors:
                print("cannot finalize: " + error, file=sys.stderr)
            return 1
    ...
    if args.command == "close":
        ...
        if not build_result.get("ok"):
            for error in build_result.get("errors", []):
                print("  " + error, file=sys.stderr)
            return 1
        consistency_errors = build_campaign_consistency_errors(report, build_result)
        ...
        report["phase4_complete"], report["phase4_remaining_actions"] = phase4_status(args.finalize)
