...
def load_module():
    spec = importlib.util.spec_from_file_location(
        "issue_campaign_v4_2_tests", HERE / "issue_campaign_v4_2.py")
...
issuer = load_module()
FREEZE_TEST_PATH = HERE / "test_freeze_contract_v4_2.py"
freeze_test_module = None
if FREEZE_TEST_PATH.is_file():
    freeze_test_spec = importlib.util.spec_from_file_location(
        "freeze_contract_v4_2_shared_fixture", FREEZE_TEST_PATH)
...
FIXTURE_HOLDOUT_CODES = ("541890", "541340", "541370", "541199", "541420")
...
class RepositoryFixture:
    def __init__(self, omit_frozen=()):
        ...
        self.plan_path = "pipeline/v4_2/plans/test-plan.json"
        self._build()
    ...
    def _build(self):
        ...
        codes = list(issuer.REGRESSION_CODES) + list(FIXTURE_HOLDOUT_CODES)
        rows = []
        for code in codes:
            ...
            dataset = {
                "dataset_version": "4.2", "snapshot_id": "fixture-%s" % code,
                "naics": code, "title": title,
            }
            self.write_json(dataset_path, dataset)
            dataset_sha = issuer.sha256_file(self.root / dataset_path)
            self.write_json("pipeline/specs_v4_2/%s.json" % code, {
                "spec_version": "4.2", "naics": code, "title": title,
                "methodology_snapshot": {
                    "path": "V4_2_RUNTIME_METHODOLOGY.md",
                    "sha256": issuer.sha256_bytes(b"frozen methodology\n"),
                },
                "dataset_snapshot": {"path": dataset_path, "sha256": dataset_sha},
            })
            self.write_bytes(
                "pipeline/prompts_v4_2/%s.md" % code,
                ("frozen prompt for %s\n" % code).encode("utf-8"),
            )
        self.write_json(issuer.TARGETS_PATH, {"codes": rows})
        self.write_json(issuer.HOLDOUT_PATH, {"selected_codes": list(FIXTURE_HOLDOUT_CODES)})
        self.write_json(issuer.REGRESSION_PATH, {
            ...
            "selected_codes": list(issuer.REGRESSION_CODES),
            "selection_rule": "The five viewed v4.0 regression codes",
        })
        for key, relative in issuer.TOOLCHAIN_PATHS.items():
            if relative == "pipeline/build/issue_campaign_v4_2.py":
                content = (HERE / "issue_campaign_v4_2.py").read_bytes()
            elif relative == issuer.MODEL_ROUTES_PATH:
                self.write_json(relative, {
                    "route_version": "v4.2-neutral-model-routes-1",
                    "research": {"target": "fixture-terra"},
        ...
        for code in codes:
            required.update({
                "pipeline/prompts_v4_2/%s.md" % code,
                "pipeline/specs_v4_2/%s.json" % code,
                "pipeline/datasets/v4_2/%s.json" % code,
            })
        ...
        self.git("tag", "-a", "v4.2-freeze-test", "-m", "v4.2 root %s" % manifest["root_sha256"])
        tag_sha = self.git("rev-parse", "refs/tags/v4.2-freeze-test^{tag}")
        common = {
        ...
        self.write_json(self.ci_receipt, {
            **common, "kind": "ci", "tag_name": "v4.2-freeze-test",
            ...
            "artifact_id": 456, "artifact_name": "v4.2-freeze-" + commit_sha,
...
class IssueCampaignV42Tests(unittest.TestCase):
    def setUp(self):
    ...
    def issue(self, scope="regression"):
        return issuer.issue(
    ...
    def test_regression_plan_and_envelopes_are_exact_and_create_no_research(self):
        plan = self.issue("regression")
        self.assertEqual(plan["plan_version"], issuer.PLAN_VERSION)
        self.assertEqual(plan["scope"], "regression")
        self.assertEqual(plan["membership"]["codes"], list(issuer.REGRESSION_CODES))
        self.assertEqual(
        ...
        for entry in plan["entries"]:
            ...
            self.assertEqual(entry["finalizer_execution"]["role"], "finalizer")
            ...
            for key in ("packet", "record", "memo", "review"):
                ...
                reservation = entry["output_reservations"][key]
                reservation_path = self.fixture.root / reservation["path"]
                self.assertTrue(reservation_path.is_file())
                self.assertEqual(issuer.sha256_file(reservation_path), reservation["sha256"])
                expected_owner = issuer._claim_execution(entry, key)["codex_task_path"]
                claim = issuer.load_json(reservation_path)
                self.assertEqual(claim["authorized_task_path"], expected_owner)
        self.assertEqual(
    ...
    def test_reviewer_visible_identifiers_are_scope_and_campaign_opaque(self):
        campaign_id = "secret-benchmark-campaign"
        plan = issuer.issue(
            self.fixture.root, "regression", campaign_id, self.run_date,
            self.fixture.freeze_path, self.fixture.plan_path,
            *self.fixture.issuance_args())
        forbidden = (campaign_id, "regression", "holdout", "full")
        for entry in plan["entries"]:
            self.assertEqual(entry["kind"], "target")
            visible = [entry["run_id"], entry["entry_id"],
                       entry["review_execution"]["codex_task_path"],
                       *entry["planned_outputs"].values()]
            envelope = issuer.load_json(
                self.fixture.root / entry["execution_envelope"]["path"])
            visible.extend((envelope["run_id"], envelope["codex_task_path"]))
            self.assertEqual(envelope["kind"], "target")
    ...
    def test_validate_issued_plan_reconstructs_every_binding_and_envelope(self):
        issued = self.issue("regression")
        issuer._verify_live_ci.side_effect = AssertionError(
            "campaign validation must not re-fetch expiring CI artifacts")
        plan, errors = issuer.validate_issued_plan(
            self.fixture.root, self.fixture.plan_path, expected_scope="regression")
        issuer._verify_live_ci.side_effect = None
    ...
    def test_global_task_namespace_rejects_cross_role_and_cross_entry_collisions(self):
        plan, _envelopes = issuer.build_issuance(
            self.fixture.root, "regression", "test-campaign", self.run_date,
    ...
    def test_model_routes_and_regression_membership_are_frozen_authorities(self):
        ...
        self.assertEqual(routes["research"], {"target": "fixture-terra"})
        self.assertNotIn("golden", routes["research"])
        self.assertEqual(routes["review"], {"all": "fixture-sol"})
        for entry in self.issue()["entries"]:
            self.assertEqual(entry["kind"], "target")
            self.assertEqual(entry["research_execution"]["model_id"], "fixture-terra")
        ...
        membership["selected_codes"] = list(reversed(issuer.REGRESSION_CODES))
    ...
    @unittest.skipUnless(freeze_test_module is not None, "v4.2 freeze fixture not yet installed")
    def test_real_freeze_contract_shared_fixture_integrates_with_issuer(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as directory:
                ...
                for code in (
                        *issuer.REGRESSION_CODES,
                        *freeze_test_module.contract_module.HOLDOUT_CODES):
                    dataset_path = "pipeline/datasets/v4_2/%s.json" % code
                    issuer_path = root / dataset_path
                    dataset = issuer.load_json(issuer_path)
                    dataset.update({
                        "dataset_version": "4.2", "snapshot_id": "shared-" + code,
                        "naics": code, "title": titles[code],
                    })
                    issuer_path.write_bytes(issuer.canonical_bytes(dataset))
                    spec_path = root / "pipeline/specs_v4_2" / (code + ".json")
                    industry_spec = issuer.load_json(spec_path)
                    industry_spec.update({
                        "spec_version": "4.2", "naics": code, "title": titles[code],
                        "dataset_snapshot": {
                            "path": dataset_path, "sha256": issuer.sha256_file(issuer_path),
                        },
                    })
                    spec_path.write_bytes(issuer.canonical_bytes(industry_spec))
                    (root / "pipeline/prompts_v4_2" / (code + ".md")).write_text(
                ...
                (root / "pipeline/v4_2/allowed_signers").write_text(
                ...
                git("config", "gpg.ssh.allowedSignersFile", "pipeline/v4_2/allowed_signers")
                git("add", ".")
                git("commit", "-qm", "exact frozen commit")
                commit_sha = git("rev-parse", "HEAD")
                tag_name = "v4.2-freeze-test"
                ...
                plan = issuer.issue(
                    root, "regression", "shared-integration", self.run_date,
                    issuer.FREEZE_MANIFEST_PATH, "pipeline/v4_2/plans/integration.json",
                ...
                validated, errors = issuer.validate_issued_plan(
                    root, "pipeline/v4_2/plans/integration.json",
                    expected_scope="regression", ci_fetcher=lambda url: responses[url],
                )
                self.assertEqual(errors, [])
    ...
    def test_real_signed_c1_c2_full_issuance_and_exact_extension_inventory(self):
        """Exercise the real C1 contract, signed tags, C2 contract, and full builder."""
        self.live_ci_patch.stop()
        self.git_authority_patch.stop()
        self.contract_patch.stop()
        try:
            with tempfile.TemporaryDirectory() as directory:
                root = Path(directory).resolve()
                fixture = freeze_test_module.Fixture(root)
                targets = issuer.load_json(root / issuer.TARGETS_PATH)["codes"]
                titles = {row["naics"]: row["title"] for row in targets}
                for code, title in titles.items():
                    dataset_path = root / "pipeline/datasets/v4_2" / (code + ".json")
                    dataset = issuer.load_json(dataset_path)
                    dataset.update({"dataset_version": "4.2", "naics": code, "title": title})
                    dataset_path.write_bytes(issuer.canonical_bytes(dataset))
                    spec_path = root / "pipeline/specs_v4_2" / (code + ".json")
                    if spec_path.is_file():
                        ...
                        industry_spec.update({
                            "spec_version": "4.2", "naics": code, "title": title,
                            "dataset_snapshot": {
                                "path": "pipeline/datasets/v4_2/%s.json" % code,
                                "sha256": issuer.sha256_file(dataset_path),
                            },
                        })
                        spec_path.write_bytes(issuer.canonical_bytes(industry_spec))
                for relative in (issuer.TOOLCHAIN_PATHS["issuer"], issuer.FREEZE_VALIDATOR_PATH):
                    (root / relative).write_bytes((HERE / Path(relative).name).read_bytes())
                # The fixture carries a non-economic semantic checker.  The real C2
                # validator still invokes it with the exact full/check/lint command.
                (root / "pipeline/build/assemble_prompts.py").write_text(
                    "#!/usr/bin/env python3\nraise SystemExit(0)\n", encoding="utf-8")
                ...
                git("init", "-q")
                git("config", "user.email", "c1-c2-integration@example.com")
                git("config", "user.name", "C1 C2 Integration")
                git("config", "gpg.format", "ssh")
                git("config", "user.signingkey", str(key))
                git("config", "gpg.ssh.allowedSignersFile", "pipeline/v4_2/allowed_signers")
                git("add", ".")
                git("commit", "-qm", "signed C1 base")
                c1 = git("rev-parse", "HEAD")
                c1_tag = "v4.2-freeze-c1-integration"
                git("tag", "-s", c1_tag, "-m", "freeze root %s" % fixture.manifest["root_sha256"])
                c1_tag_oid = git("rev-parse", "refs/tags/%s^{tag}" % c1_tag)
                manifest_sha = issuer.sha256_file(fixture.manifest_path)
                common = {
                    "receipt_version": "v4.2-authorization-receipt-1",
                    "root_sha256": fixture.manifest["root_sha256"],
                    "manifest_sha256": manifest_sha, "commit_sha": c1,
                }
                base_commit = {**common, "kind": "commit",
                               "committed_at": "2026-01-01T00:00:00Z"}
                base_tag = {
                    **common, "kind": "tag", "tag_name": c1_tag,
                    "tag_object_sha": c1_tag_oid, "annotated": True, "signed": True,
                    "signature_verified": True, "signer_identity": "jkee",
                    "tagged_at": "2026-01-01T00:01:00Z",
                }
                api = issuer.GITHUB_API_ROOT
                base_ci = {
                    **common, "kind": "ci", "tag_name": c1_tag,
                    "provider": "github-actions", "github_owner": "jkee",
                    "github_repository": "rollup-economy",
                    "workflow_path": issuer.FREEZE_WORKFLOW_PATH, "workflow_id": 789,
                    "run_id": 123, "run_attempt": 1,
                    ...
                    "run_started_at": "2026-01-01T00:02:00Z",
                    "run_updated_at": "2026-01-01T00:03:00Z",
                    "artifact_id": 456, "artifact_name": "v4.2-freeze-" + c1,
                ...
                base_paths = [root / "external-c1-commit.json", root / "external-c1-tag.json",
                              root / "external-c1-ci.json"]
                for path, value in zip(base_paths, (base_commit, base_tag, base_ci)):
                    path.write_bytes(issuer.canonical_bytes(value))

                excluded = set(issuer.REGRESSION_CODES) | set(
                    freeze_test_module.contract_module.HOLDOUT_CODES)
                full_codes = sorted(set(titles) - excluded)
                extension_files = []
                methodology_sha = issuer.sha256_file(root / "V4_2_RUNTIME_METHODOLOGY.md")
                for code in full_codes:
                    dataset_relative = "pipeline/datasets/v4_2/%s.json" % code
                    values = (
                        ("pipeline/specs_v4_2/%s.json" % code, issuer.canonical_bytes({
                            "spec_version": "4.2", "naics": code, "title": titles[code],
                            "methodology_snapshot": {
                                "path": "V4_2_RUNTIME_METHODOLOGY.md", "sha256": methodology_sha},
                            "dataset_snapshot": {"path": dataset_relative,
                                                 "sha256": issuer.sha256_file(root / dataset_relative)},
                        })),
                        ("pipeline/prompts_v4_2/%s.md" % code,
                         ("synthetic C2 prompt %s\n" % code).encode("utf-8")),
                    )
                    for relative, content in values:
                        path = root / relative
                        path.parent.mkdir(parents=True, exist_ok=True)
                        path.write_bytes(content)
                        extension_files.append({"path": relative, "sha256": issuer.sha256_bytes(content),
                                                "byte_length": len(content)})
                extension_files.sort(key=lambda item: item["path"].encode("utf-8"))
                extension = {
                    "extension_version": "v4.2-full-scope-extension-1",
                    "purpose": "authorize-post-gate-full-scope-inputs",
                    "base_freeze_root_sha256": fixture.manifest["root_sha256"],
                    "codes": full_codes, "files": extension_files,
                    "root_sha256": issuer._root_sha256(extension_files),
                }
                extension_path = root / issuer.SCOPE_EXTENSION_MANIFEST_PATH
                extension_path.parent.mkdir(parents=True, exist_ok=True)
                extension_path.write_bytes(issuer.canonical_bytes(extension))
                git("add", issuer.SCOPE_EXTENSION_MANIFEST_PATH,
                    "pipeline/specs_v4_2", "pipeline/prompts_v4_2")
                git("commit", "-qm", "signed exact C2 extension")
                c2 = git("rev-parse", "HEAD")
                c2_tag = "v4.2-full-scope-c2-integration"
                git("tag", "-s", c2_tag, "-m", "extension-root %s\nbase-commit %s" %
                    (extension["root_sha256"], c1))
                c2_tag_oid = git("rev-parse", "refs/tags/%s^{tag}" % c2_tag)
                extension_ci_relative = "external-c2-ci.json"
                extension_ci = {
                    "root_sha256": extension["root_sha256"],
                    "manifest_sha256": issuer.sha256_file(extension_path),
                    "commit_sha": c2, "tag_name": c2_tag,
                }
                (root / extension_ci_relative).write_bytes(issuer.canonical_bytes(extension_ci))
                extension_receipt_relative = "external-c2-receipt.json"
                extension_receipt = {
                    "receipt_version": "v4.2-full-scope-extension-receipt-1",
                    "kind": "full_scope_extension",
                    "manifest_sha256": issuer.sha256_file(extension_path),
                    "root_sha256": extension["root_sha256"], "commit_sha": c2,
                    "tag_name": c2_tag, "tag_object_sha": c2_tag_oid,
                    "signed": True, "signature_verified": True, "signer_identity": "jkee",
                    "ci_receipt": issuer._artifact_ref(root, extension_ci_relative),
                }
                (root / extension_receipt_relative).write_bytes(
                    issuer.canonical_bytes(extension_receipt))
                ...
                def build(scope="full"):
                    return issuer.build_issuance(
                        root, scope, "c1-c2-integration", self.run_date,
                        issuer.FREEZE_MANIFEST_PATH,
                        *(str(path) for path in base_paths), verify_live_ci=False,
                        regression_gate_certificate=issuer.REGRESSION_GATE_CERTIFICATE_PATH
                        if scope == "full" else None,
                        holdout_gate_certificate=issuer.HOLDOUT_GATE_CERTIFICATE_PATH
                        if scope == "full" else None,
                        gate_validator=(lambda _manifest, _root: []) if scope == "full" else None,
                        scope_extension_manifest=issuer.SCOPE_EXTENSION_MANIFEST_PATH
                        if scope == "full" else None,
                        scope_extension_receipt=extension_receipt_relative
                        if scope == "full" else None)

                plan, envelopes = build()
                self.assertEqual(len(plan["entries"]), 53)
                self.assertEqual(len(envelopes), 53)
                self.assertEqual(plan["membership"]["scope_extension"]["commit_sha"], c2)
                sample = root / extension_files[0]["path"]
                original = sample.read_bytes()
                sample.write_bytes(original + b"tampered")
                with self.assertRaisesRegex(issuer.IssueError, "signed manifest|differs"):
                    build()
                sample.write_bytes(original)
                sample.unlink()
                with self.assertRaisesRegex(issuer.IssueError, "missing|inventory|required"):
                    build()
                sample.write_bytes(original)
                extra = root / "pipeline/specs_v4_2/999999.json"
                extra.write_text("{}", encoding="utf-8")
                with self.assertRaisesRegex(issuer.IssueError, "inventory"):
                    build()
        finally:
            self.contract_patch.start()
            self.git_authority_patch.start()
    ...
    def test_actual_git_objects_not_receipt_booleans_authorize_issuance(self):
        ...
        with mock.patch.object(issuer, "_git", side_effect=allow_fixture_signature) as git_mock:
        ...
        self.assertIn(
            mock.call(
                ...
                "verify-tag", "--raw", tag["tag_object_sha"],
            ),
            git_mock.call_args_list,
        )

        self.fixture.git("tag", "-d", tag["tag_name"])
    ...
    def test_external_receipts_are_supported_and_validation_is_toctou_closed(self):
        with tempfile.TemporaryDirectory() as receipt_directory:
            ...
            plan = issuer.issue(
                self.fixture.root, "regression", "external-receipts", self.run_date,
    ...
    def test_live_github_ci_proof_is_exact_and_fails_closed(self):
        ...
        artifact = {
            "id": 456, "name": "v4.2-freeze-" + commit["commit_sha"],
    ...
    def test_refuses_plan_envelope_or_research_output_overwrite(self):
        ...
        try:
            planned, _ = issuer.build_issuance(
                other.root, "regression", "test-campaign", self.run_date,
                other.freeze_path, *other.issuance_args())
            packet = other.root / planned["entries"][0]["planned_outputs"]["packet"]
    ...
    def test_atomic_failure_rolls_back_new_envelopes_and_plan(self):
        ...
        with mock.patch.object(issuer, "_atomic_create", side_effect=fail_second):
            with self.assertRaisesRegex(OSError, "synthetic atomic failure"):
                self.issue()
        self.assertFalse((self.fixture.root / self.fixture.plan_path).exists())
        envelopes = list((self.fixture.root / "pipeline/v4_2/envelopes").rglob("*.json"))
        self.assertEqual(envelopes, [])
        self.assertEqual(list(self.fixture.root.rglob("*.reservation.json")), [])
    ...
    def test_output_race_before_plan_commit_rolls_back_all_envelopes(self):
        ...
        def inject_packet(root, relative, content):
            ...
            if raced["path"] is None and "/envelopes/" in relative:
                plan = issuer.load_json(root / self.fixture.plan_path) if (root / self.fixture.plan_path).is_file() else None
                if plan is None:
                    plan, _ = issuer.build_issuance(
                        root, "regression", "test-campaign", self.run_date,
                        self.fixture.freeze_path, *self.fixture.issuance_args())
                packet_relative = plan["entries"][0]["planned_outputs"]["packet"]
        ...
        self.assertFalse((self.fixture.root / self.fixture.plan_path).exists())
        self.assertEqual(
            list((self.fixture.root / "pipeline/v4_2/envelopes").rglob("*.json")), [])
        self.assertEqual(list(self.fixture.root.rglob("*.reservation.json")), [])

    def test_preexisting_output_claim_is_never_overwritten(self):
        plan, _envelopes = issuer.build_issuance(
            self.fixture.root, "regression", "test-campaign", self.run_date,
            self.fixture.freeze_path, *self.fixture.issuance_args(),
        )
        claim_relative = plan["entries"][0]["output_reservations"]["packet"]["path"]
        claim = self.fixture.root / claim_relative
        claim.parent.mkdir(parents=True, exist_ok=True)
        claim.write_text("foreign claim", encoding="utf-8")
        with self.assertRaisesRegex(issuer.IssueError, "refusing to overwrite"):
            self.issue()
        self.assertEqual(claim.read_text(encoding="utf-8"), "foreign claim")
        self.assertFalse((self.fixture.root / self.fixture.plan_path).exists())
        self.assertEqual(list((self.fixture.root / "pipeline/v4_2/envelopes").rglob("*.json")), [])

    def test_issuer_credentials_cli_requires_pipe_and_delivers_ephemerally(self):
        argv = [
            "--repo-root", str(self.fixture.root), "--scope", "regression",
            "--campaign-id", "pipe-test", "--run-date", self.run_date,
            "--freeze-manifest", self.fixture.freeze_path,
            "--commit-receipt", self.fixture.commit_receipt,
            "--tag-receipt", self.fixture.tag_receipt,
            "--ci-receipt", self.fixture.ci_receipt,
            "--output", self.fixture.plan_path,
        ]
        fake = ({"entries": [], "plan_sha256": "a" * 64},
                {"entry": {"packet": "private-token"}})
        read_fd, write_fd = os.pipe()
        try:
            with mock.patch.object(issuer, "issue_with_credentials", return_value=fake):
                self.assertEqual(issuer.main(argv + ["--credentials-fd", str(write_fd)]), 0)
            os.close(write_fd)
            write_fd = None
            payload = os.read(read_fd, 10000)
            self.assertIn(b"private-token", payload)
        finally:
            os.close(read_fd)
            if write_fd is not None:
                os.close(write_fd)

        regular_path = self.fixture.root / "scratch" / "credentials.json"
        regular_path.parent.mkdir(parents=True, exist_ok=True)
        regular = os.open(regular_path, os.O_CREAT | os.O_RDWR, 0o600)
        try:
            with mock.patch.object(issuer, "issue_with_credentials") as issue_mock:
                self.assertEqual(issuer.main(argv + ["--credentials-fd", str(regular)]), 1)
                issue_mock.assert_not_called()
        finally:
            os.close(regular)

    def test_credential_delivery_failure_rolls_back_and_retry_succeeds(self):
        argv = [
            "--repo-root", str(self.fixture.root), "--scope", "regression",
            "--campaign-id", "delivery-retry", "--run-date", self.run_date,
            "--freeze-manifest", self.fixture.freeze_path,
            "--commit-receipt", self.fixture.commit_receipt,
            "--tag-receipt", self.fixture.tag_receipt,
            "--ci-receipt", self.fixture.ci_receipt,
        ...
        def assert_no_issued_state():
            self.assertFalse((self.fixture.root / self.fixture.plan_path).exists())
            self.assertEqual([], list((self.fixture.root / "pipeline/v4_2").rglob("*.reservation.json")))
            self.assertEqual([], list((self.fixture.root / "pipeline/v4_2").rglob("*.claim.lock")))
            self.assertEqual([], list((self.fixture.root / "pipeline/v4_2").rglob("*.claim-receipt.json")))
            envelope_root = self.fixture.root / "pipeline/v4_2/envelopes"
            self.assertEqual([], list(envelope_root.rglob("*.json")) if envelope_root.exists() else [])

        read_fd, write_fd = os.pipe()
        os.close(read_fd)
        try:
            self.assertEqual(issuer.main(argv + ["--credentials-fd", str(write_fd)]), 1)
        finally:
            os.close(write_fd)
        assert_no_issued_state()

        read_fd, write_fd = os.pipe()
        try:
            with mock.patch.object(issuer.os, "write", side_effect=[7, BrokenPipeError("partial")]):
                self.assertEqual(issuer.main(argv + ["--credentials-fd", str(write_fd)]), 1)
        finally:
            os.close(read_fd)
            os.close(write_fd)
        ...
        try:
            ...
            os.close(write_fd)
            write_fd = None
            delivered = os.read(read_fd, 65536)
            self.assertIn(b"claim_credentials", delivered)
        finally:
            os.close(read_fd)
            if write_fd is not None:
                os.close(write_fd)
        self.assertTrue((self.fixture.root / self.fixture.plan_path).is_file())
