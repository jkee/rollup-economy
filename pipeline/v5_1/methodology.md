# v5.1 methodology — AI roll-up industry opportunity screen

**Status:** frozen. Copied from the frozen v5.0 contract with exactly these
amendments: version identity 5.1; fleet scope = the full 1,012-code universe
in `targets.json` (§7); sampled validation (§§2, 5, 8); v5_1 paths (§10).
Every other formula, anchor, cut, evidence rule, and brief substance is
byte-inherited from v5.0. The freeze identity is the git commit that
completes this folder; every artifact records the runtime commit
...
(`6digit/six_data_v5.json`) remains immutable history. The v5.1 output is a
**provisional sampled-validation screen** until a full-validation campaign
publishes its successor.
**Campaign contract:** `V5_1_PLAN.md` (block loop, sampling contract,
approval gates, pause trigger); mutable state in `campaign_state.json`.
...
deterministic code (build.py + score.py)  — validation + arithmetic
        ↓
independent validator model (validator_brief.md) — sampled attempts only
        ↓
publish accepted / publish not_reviewed / exclude   (build.py site)
```
...
**v5.1 amendment — sampled validation.** An isolated validator model reviews
every **sampled** attempt under `validator_brief.md`; sample membership is
computed by code under the frozen sampling contract in `V5_1_PLAN.md`
(mandatory stratum: every base-tier `HIGHEST_PRIORITY`/`PRIORITY` record;
deterministic seeded random stratum; canary reviewed 100%) and is never
chosen by a session. Every published record carries
`independent_review: accepted | not_reviewed`; deterministically unsampled
records publish as `not_reviewed` by design. The validator returns
`publishable`, `publishable_with_caveats`, `reject`, or
...
3. **Canary**: research the ten frozen cross-sector codes (`238910`,
   `311612`, `332710`, `423840`, `445110`, `484110`, `522310`, `621111`,
   `111998`, `921190`) fresh under v5.1; score; one isolated validator per
   record (the canary is reviewed at 100%, purposive, never counted toward
   any random stratum); one bounded remediation per `reject`/`invalid`;
   then **stop for human approval** (`canary_approved` in
   `campaign_state.json`). The canary fails for systemic construct errors,
   misleading evidence treatment, broken mechanics, or results useless for
   prioritization — not because estimates remain, ranges cross tiers, or
   goods sectors screen out en masse. If the frozen lens proves incoherent
   for a territory, that territory is descoped with the reason stated —
   never patched mid-campaign.
4. **Fleet** (after approval only): the fleet scope is the full 1,012-code
   universe in `targets.json`, run block by block per `V5_1_PLAN.md`; the
   ten canary codes count toward their home blocks. One run per industry,
   one review per sampled attempt, one remediation wave per block, publish
   per the sampled-validation contract, exclude and report the rest, close.
   Expanding or shrinking the scope is a new version.

## 8. Standing rules

1. Never hand-edit a finalized `score.json`, `memo.md`, or the site dataset.
2. Never publish outside the fail-closed sampled-validation contract:
   deterministically **unsampled** records publish as `not_reviewed` by
   design, but no path publishes a **sampled** record without its accepted
   review or reviewed exclusion. There is no `--allow-unreviewed` build.
...
pipeline/v5_1/
  ...
  score.py                      pure arithmetic (inherited verbatim from v5.0)
  build.py                      validate / finalize / check / validate-review /
                                audit-state / site
  assignment.py                 research / review / sample launchers
  targets.json                  frozen fleet scope: 1,012 codes, blocks, canaries
  campaign_state.json           mutable campaign state: approvals, block statuses,
                                sample assignments (the only mutable file here)
  test_v5_1.py                  sentinels and contract tests
  runs/<naics>/<run_id>/        packet.json, score.json, memo.md, review.json
6digit/six_data_v5_1.json       generated provisional site dataset — never hand-edited
```
