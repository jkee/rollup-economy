...
- [ ] 4.8 **v3.1.1 runner/schema hardening before any broader rerun.** Minimal changes only: define `DERIVED` as fully reproducible arithmetic/unit normalization with no judgmental bridge; require `ESTIMATE` whenever any mapping step is judgmental; mechanically reject ellipses in `figure_quoted`; replace free-text citation scope with required population/geography/unit/date/mismatch fields (allow explicit `not reported`). Re-run a new canary series and require nonzero acceptance plus no systemic defect before launching the remaining fleet. Do not alter thresholds or scoring rubric.
