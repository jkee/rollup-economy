...
- [~] 4.6 **Triggered by the 0/72 acceptance result:** audit a small representative rejection sample to separate unreachable/fabricated URLs, reachable-but-non-supporting citations, prompt/source-contract failures, validator-policy mismatches, and genuine rubric/judgment errors. If the instrument failed: fix template → bump to v3.1 → re-freeze → re-run a canary before resuming the pilot. Never hand-fix records or relax the rubric post hoc.
