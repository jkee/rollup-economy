...
- [~] 4.7 **v3.1 five-record canary authorized and started 2026-07-20.** The reviewed contract is frozen at commit `9e7239b`; all 63 assembled prompts are frozen at `f4ba726`. A reproducible random sample from the 63 Stage-4 fleet targets (`random.Random("v3.1-canary-1")`) selected `524210 541110 541350 541612 541613`. The configured subagent runtime did not expose GPT-5.5, so the research canary used the available balanced fleet model `gpt-5.6-terra` and recorded that exact model ID. Research ran in isolated 2+2+1 lanes after a two-record canary gate; all five drafts and final runs pass the orchestrator's independent draft schema, final schema, fresh-finalization equality, identity, atomic-URL-path and exact arithmetic checks (24 citation occurrences total; zero score deltas). Preliminary deterministic verdicts are 2 conditional / 1 pass / 2 kill, all LOW confidence; these are not accepted until full-depth isolated Sol validation finishes. Do not launch the remaining fleet or rewrite existing records before this canary passes.
