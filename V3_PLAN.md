...
- [~] 4.7 **v3.1 five-record canary authorized and started 2026-07-20.** The reviewed contract is frozen at commit `9e7239b`. A reproducible random sample from the 63 Stage-4 fleet targets (`random.Random("v3.1-canary-1")`) selected `524210 541110 541350 541612 541613`. Assemble and validate `prompts_v3_1/`, run isolated GPT-5.5 research lanes (2+2+1), independently schema/recompute every result, then run full-depth isolated Sol validation on all five. Do not launch the remaining fleet or rewrite existing records before this canary passes.
