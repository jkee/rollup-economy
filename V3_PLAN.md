...
- [~] 4.8 **v3.1.1 hardening started 2026-07-21; non-prompt checkpoint complete.** Frozen v3.1 records remain untouched. The new versioned draft/final schemas and Python finalizer separate reusable atomic evidence facts from selected inputs; replace model-authored provenance with `OBSERVED | CALCULATED | ESTIMATE` methods that Python maps to final provenance; permit `CALCULATED` only with a safe machine-recomputed formula that actually uses fact-backed operands; require structured population/geography/unit/period/industry-mismatch scope; reject compact, Unicode and spaced ellipses in exact quotes; and validate fact IDs, evidence-quality consistency, branch-specific fields, real ISO dates, finite numbers, disclosure structure and dataset-injected role identity/shares. `build.py` independently rechecks the finalized contract, `review_campaign.py` routes v3.1.1 to its future prompt path and exact `validator-3.1.1` binding, and older versions retain their prior semantics. Fifteen focused v3.1.1 tests plus all 45 inherited build/review tests pass; frozen prompt assembly (63/63) and golden separation also pass. No prompt, draft, industry run, review, manifest or publication artifact was generated. Prompt/brief changes—retaining C1–C4—and industry reruns remain a later checkpoint. Re-run a new canary series and require nonzero acceptance plus no systemic defect before launching the remaining fleet. Do not alter thresholds or scoring rubric.
