...
**Status: living document, incomplete** — captures how to re-run the project end-to-end, which model runs what and why, and what it costs. Written after the sector-54 pilot (2026-07-20); sections marked TBD await the validator pass and the Phase-6 API runner. Companions: [V3_PRODUCT.md](V3_PRODUCT.md) (what and why), [V3_PLAN.md](V3_PLAN.md) (progress tracker).
...
Stage 2  prompt gen      blocks/<naics>.json → prompts/<naics>.md  BEST model    re-run: on template version bump
Stage 3  research runs   prompts → runs/<naics>/<runid>.json       CHEAP model   re-run: staleness (~12mo) or 'wrong'
Stage 4  build           runs → six_data_v3.json + stats + flags   no model      re-run: on any input change (seconds)
Stage 5  acceptance      records → review/<naics>.json             BEST model    DEFERRED — see §6
```
...
| Per-code prompt blocks (Stage 2) | **Fable-class** | One-time per code; errors here propagate into every downstream run; needs real industry judgment |
| Golden-set reference runs | **Fable-class** | 20 codes anchoring the instrument; the quality ceiling everything is benchmarked against |
| Fleet research runs (Stage 3) | **Sonnet-class** | Proven adequate: 63/63 schema-clean, mechanisms match Fable runs on shared codes; ~5× cheaper |
| Acceptance validator (Stage 5) | **Fable-class** (policy TBD: full vs risk-tiered, see §6) | Judgment about judgment; catches plausible-but-wrong |
| Re-runs of `wrong` records | Sonnet-class (escalate to Fable on second failure) | Same as fleet |

Observed Sonnet-vs-Fable behavior on the 20 shared benchmark codes: **structure and mechanisms agree** (same moats, same capture traps, same melter mechanisms found independently); divergence concentrates in the declared-uncertain inputs (t50, buy/exit multiples), Sonnet systematically more conservative on adoption timing. Two terminal-class boundary disagreements in 20 (translation, court reporting). Conclusion: Sonnet fleet + Fable anchors is a sound trade; do not downgrade prompt-gen or golden runs.

## 3. Cost assumptions (observed in the pilot, blended in/out tokens per unit)

| Unit | Model | Observed tokens | Notes |
|---|---|---|---|
| 1 research run (1 code) | Sonnet | ~50–60K | single-author s2 runs, incl. web research + self-validation loop |
| 1 golden run (1 code) | Fable | ~50–55K | same shape as fleet run |
| 1 prompt block (1 code) | Fable | ~8–9K | 13 blocks ≈ 110K per writer agent |
| 1 acceptance review (1 record) | Fable | ~25–35K **(estimate)** | not yet run; source-verification web fetches dominate |
| Build / datasets / analysis | — | ~0 | Python, seconds |

Pricing assumption used for planning: **Fable ≈ 5× Sonnet per token** (verify against current price list before Phase 6).

**Pilot actuals (83 records):** golden 20 × ~52K Fable + fleet 63 × ~55K Sonnet + prompt-gen 63 codes Fable + meta/assembler/datasets agents. **Waste line:** the failed first fleet wave (delegation cascades, see §5) burned roughly a full fleet's worth of Sonnet tokens for zero usable records — prevented in the s2 design, but budget a 10–20% incident margin anyway.

**Phase-6 extrapolation (1,012 codes, order-of-magnitude):**
- Fleet: 1,012 × ~55K Sonnet ≈ 55M Sonnet-tokens
- Prompt-gen: 1,012 × ~9K Fable ≈ 9M Fable-tokens
- Validator full-Fable: 1,012 × ~30K ≈ 30M Fable-tokens — **the dominant cost item** (≈ 3× the fleet in Sonnet-equivalents)
- Validator risk-tiered (~35% Fable depth, Sonnet pre-screen for deep-kill): cuts the validator line ~50–60%
- API-price sanity: at Sonnet ~$3/$15 per M and Fable ≥ Opus pricing, expect fleet in the low hundreds of $ and the validator as the swing item from hundreds to ~$1K+ depending on the tiering decision.
...
**Golden runs:** same brief, Fable model, output to `pipeline/golden/<naics>.json`, run_id `golden-<series>-<naics>`, and the isolation clause additionally bans reading `golden_set.json` (it contains expected outcomes — contamination).

## 5. Incident lessons (encoded, do not relearn)

1. **Delegation cascades destroy runs.** Sonnet agents given 3 codes spontaneously fanned out research sub-agents 3–4 levels deep: ~5–10× token burn, and every assembled record failed schema (invented key names, missing per-input fields). Fix: the no-delegation clause + 2-code briefs + the real-validator self-check loop. On the API (Phase 6) this is structurally impossible — one call, one record.
...
- **Stage 5 validator playbook** — TBD. Pending decision: full-Fable on all records vs risk-tiered (Fable for conditional/borderline/LOW-confidence/golden ≈ 35% of records; Sonnet pre-screen with escalate-on-suspicion for the deep-kill tail). Validator brief must include: verify cited URLs resolve and contain the quoted figures; hunt memory-based sourcing (inputs whose source can't be found at the URL); judgment inputs within rubric and consistent across comparable industries; cross-checks answered honestly; verdict `accepted`/`wrong` + reasons per review_record.schema.json.
...
- **Phase-6 API batch runner** — TBD. One API call per code (Sonnet + web search tool), schema-validated response, automatic retry with error attached, then build + validator. Design goals: no delegation possible, resumable, per-sector cost tracking.
...
python3 pipeline/build/test_build.py           # regression suite (10 tests)
```
